...
## 5-a. Espaçamento dos formulários em grade (ficha §1, item 10)

Três pontos montam grade de formulário com estilo inline `gap: "0 16px"` — row-gap zero, rótulo colando no campo de cima: `patrocinadores/formulario-patrocinador.tsx:40`, `patrocinadores/[id]/formularios-contrato.tsx:55` e `campanhas/[id]/modelagem.tsx:303`. **Crie uma classe de grade de formulário** no `dseed-admin.css` (bloco `Extensão (Onda 14)`): duas colunas, row-gap **14px** (a régua do `pm-grid`), column-gap 16px, e o colapso mobile continua vindo do `.g-resp` existente — confira que a classe nova não conflita com o `!important` dele. Substitua o estilo inline nos três pontos pela classe; campos de largura total seguem com `gridColumn: "1 / -1"` ou ganham utilitária, a seu critério declarado. Screenshot antes/depois da T32 no PR, e2e de responsividade continua verde.
