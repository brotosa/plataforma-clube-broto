...
7. **RN07** — Telemetria (vouchers, compras) é somente leitura, com origem, data e arquivo de importação rastreáveis. **Errata de 26/08 — fonte do card:** o card "Telemetria por oferta" (T5) passou a mostrar o dado da **operadora** (Onda 12) — **catálogo** ("Lista de Ofertas") e **extrato** ("Resgate e Compras", casado por `id_oferta`) em blocos separados, **nunca somados** (RN68), cada um com origem e data. O voucher clássico (Onda 1, `telemetriaEvento`) continua exibido, mas **só quando tiver dado** — em produção ele nunca foi alimentado, e a fonte real de resgate é a operadora. Sem nenhuma das três, o card mantém o estado de ausência (nada estimado).
